...
sys.path.append(".")
#sys.path.append("../../../stnerf/instant-ngp/build")
#sys.path.append("../../../stnerf/instant-ngp/scripts")
#sys.path.append(".")
...
def infer_npg(args, frame, param, t_step):
    ...
    with tqdm(range(testbed.nerf.training.dataset.n_images), unit="images", desc=f"Rendering test frame") as t:
        for i in t:
            ...
            save_path = os.path.join(save_root_path, f'cams_normals{args.suffix}')
            if not os.path.isdir(save_path):
                os.makedirs(save_path) 
            save_path = os.path.join(save_root_path, f'cams_normals{args.suffix}', f'{frame:08d}.png')
...
if __name__ == "__main__":
    ...
    save_path = os.path.join(save_root_path, f'cams_normals{args.suffix}')      
    rate = 30#*t_step 
    os.system(f"ffmpeg -y -framerate {rate} -i {save_path}/%08d.png -b:v 20M -c:v libx264 {save_root_path}/cams_normals{args.suffix}.mp4")
    f.close()
